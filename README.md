<p align="center">
  <img
    src="./LuJo030-GitHub-Banner-Welle.png"
    alt="LuJo030 – Discord-Bots, Webentwicklung und Servertechnik"
    width="100%"
  >
</p>

<h1 align="center">Hallo, ich bin Joel</h1>

<p align="center">
  Entwickler von Discord-Bots, Web-Dashboards und Serverlösungen
</p>

<p align="center">
  <a href="https://elytrix.de">
    <img src="https://img.shields.io/badge/Website-elytrix.de-16c8f4?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Website">
  </a>

<p align="center">
  <a href="https://github.com/LuJo030">
    <img src="https://img.shields.io/badge/GitHub-LuJo030-7c5cff?style=for-the-badge&logo=github&logoColor=white" alt="GitHub">
  </a>
</p>

---

## Über mich

Ich bin Joel und auf GitHub als `LuJo030` unterwegs.

Ich beschäftige mich hauptsächlich mit der Entwicklung von Discord-Bots, Web-Dashboards, Linux-Systemen und eigener Serverinfrastruktur. Zusätzlich arbeite ich an Automatisierungen mit Raspberry Pi und verschiedenen Smart-Home-Komponenten.

- Ausbildung zum Informationselektroniker
- Entwickler und Betreiber von Elytrix
- Interesse an Linux, Netzwerken und Servertechnik
- Entwicklung eigener Web- und Automatisierungslösungen

## Technologien

<p>
  <img src="https://skillicons.dev/icons?i=python,java,js,nodejs,html,css,linux,git,github,raspberrypi,vscode" alt="Technologien">
</p>

## Hauptprojekt

### [Elytrix](https://elytrix.de)

Elytrix ist eine Plattform zur zentralen Verwaltung moderner Discord-Server. Sie verbindet einen leistungsfähigen Discord-Bot mit einem übersichtlichen Web-Dashboard.

**Funktionen:**

- Ticket- und Supportsystem
- Moderation und Protokollierung
- Temporäre Sprachkanäle
- Teamverwaltung
- Rollen- und Verifizierungssysteme
- Automatische Benachrichtigungen
- Zentrales Web-Dashboard
- Premium- und Lizenzsystem
- Verwaltung mehrerer Discord-Server

<p>
  <a href="https://elytrix.de">
    <img src="https://img.shields.io/badge/Elytrix-Website_öffnen-16c8f4?style=for-the-badge" alt="Elytrix öffnen">
    <a href="https://discord.elytrix.de">
  </a>
    
  <a href="https://elytrix.de">
   <img src="https://img.shields.io/badge/Discord-Server_öffnen-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="Discord öffnen">
  </a>
  
</p>

## Aktuelle Schwerpunkte

- Weiterentwicklung von Elytrix
- Entwicklung neuer Discord-Module
- Verbesserung des Web-Dashboards
- Aufbau einer eigenen Serverinfrastruktur
- Automatisierung mit Linux und Raspberry Pi
- Verbesserung von Sicherheit und Benutzerverwaltung

## GitHub-Statistiken

<p align="center">
  <img
    src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=LuJo030&theme=tokyonight"
    alt="GitHub-Profilübersicht"
    width="100%"
  >
</p>

<p align="center">
  <img
    src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=LuJo030&theme=tokyonight"
    alt="GitHub-Statistiken"
    width="49%"
  >
  <img
    src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=LuJo030&theme=tokyonight"
    alt="Verwendete Programmiersprachen"
    width="49%"
  >
</p>
